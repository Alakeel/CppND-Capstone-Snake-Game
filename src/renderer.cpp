...
bool Renderer::Render(std::shared_ptr<Snake> &snake, SDL_Point const &food)
{
  SDL_Rect block;
  ...
  if(!snake->Alive())
  {
  std::string msgResults {"          * Game Over *         \nClick OK to display the results."};
  char cstr[msgResults.size() + 1];
  strcpy(cstr, msgResults.c_str());
